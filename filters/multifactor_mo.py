...
def run_monthly_multifactor_filter(df: pd.DataFrame, index_df: pd.DataFrame) -> pd.DataFrame:
    # Apply all four monthly filters
    momentum = run_monthly_momentum_filter(df)
    rs = run_monthly_rs_filter(df, index_df)
    accumulation = run_monthly_accumulation_filter(df)
    price_action = run_monthly_price_volume_filter(df)
